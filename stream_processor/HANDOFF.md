...
```
(:User {user_id})-[:BOUGHT]->(:Product {product_id})
```
Recommendation query ("customers who bought X also bought Y"):
```cypher
MATCH (:Product {product_id: $pid})<-[:BOUGHT]-(u:User)-[:BOUGHT]->(rec:Product)
WHERE rec.product_id <> $pid
RETURN rec.product_id, count(*) AS strength
ORDER BY strength DESC LIMIT 5
```
Verified working via Neo4j Browser table view — real `User -> Product` pairs
confirmed (e.g. `USR578 -> P40`, `USR188 -> P22`).

## How to run this locally

1. Start all services from the **repo root** (not any subfolder):
   ```bash
   cd shopsphereanalytics
   docker compose up -d
   docker ps   # confirm 6 containers: kafka, zookeeper, mongodb, cassandra, redis, neo4j
   ```
2. **Wait ~45 seconds** before connecting to Cassandra — it takes a while to
...
3. Install dependencies:
   ...
   ```
4. Seed product names (one-time):
   ```bash
   python seed_products.py   # or: py seed_products.py on Windows
   ```
5. Run Person A's producer (separate terminal) and this consumer together:
   ```bash
   # terminal 1
   ...
   ```
6. Verify data landed (see verification commands per database above, or just
...
1. **Port 27017 conflict on Windows:** if you have MongoDB installed natively
...
2. **Cassandra driver fails to import on Python 3.12+ (including 3.14):**
   the driver's default connection class relies on the `asyncore` module,
   which was removed from the standard library in 3.12. Fix: `pip install
   pyasyncore` — this restores an importable `asyncore` shim and the driver's
   normal auto-detection succeeds without any code changes needed.
3. **Data does not persist across `docker compose down` by default** — the
...
4. **`docker compose down` + `up` needs a run from the repo root** — running
